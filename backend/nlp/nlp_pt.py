...
class Natural_language:
	...
	#Entrada: String s1 e s2 (unicode); no Python 3, str é unicode
	...
	#Retorno: Verificar se é necessario devolver lista de listas de tuplas (onde cada lista corresponde a uma sentença)
	...
	#Parâmetros: tagged (lista de tuplas). Cada tupla é formada por uma palavra e sua respectiva tag sintática.
	...
	def chunk_noun_phrase(self, tagged):

		chunkGram = r"""NP: {<DET>?<NOUN|PROPN>+<ADJ>*}
						NP: {<NP><ADP><NP>}
						NP: {<NP><ADP><NP>}
						NP: {<NP><ADP><NP>}
						NP: {<NP><ADP><NP>}
	...
	def read_tree(self, node, list):

		if isinstance(node, tuple):
			list.append(node)
		else:
			for i in range(len(node)):
				self.read_tree(node[i], list)
	...
	def get_noun_phrase(self, chunked):
			
		list = []
		for node in chunked.subtrees(filter=lambda x: x.label() == 'NP'):
			res = []
			self.read_tree(node, res)
			list.append(res)
	...
	# pln = spacy.load('pt')
	# doc = pln(u'número complexo parte imaginária números complexos partes imaginárias numero imaginaria')
	# tokens = [token for token in doc]
	# print("Similaridade token a token")
	# for i in range(len(tokens)):
		# print(tokens[i],': ', tokens[0].similarity(tokens[i]))
		
		
	# nl = Natural_language()
	# s1 = u'número complexo'
	# s2 = u'parte real'
	# print(nl.semantic_similarity(s1, s2))
	
	
	#você deve declarar a string como unicode para que ele funcione corretamente.
	#tokens = nlp.syntactic_analysis(u'Você encontrou o livro que eu te falei, Carla?')
	#tokens = nlp.syntactic_analysis(u'Um algoritmo genético (AG) é uma técnica de busca utilizada na ciência da computação para achar soluções aproximadas em problemas de otimização e busca, fundamentado principalmente pelo americano John Henry Holland. Algoritmos genéticos são uma classe particular de algoritmos evolutivos que usam técnicas inspiradas pela biologia evolutiva como hereditariedade, mutação, seleção natural e recombinação (ou crossing over).')
	#tree_chunked = nlp.chunk_noun_phrase(tokens)
	#list_chunked = nlp.get_noun_phrase(tree_chunked)
	#print(tokens)
	#print(list_chunked)
...
#**********ABAIXO ESTÃO OS CÓDIGOS QUE PODEM SER REAPROVEITADOS***************
